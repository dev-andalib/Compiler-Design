...
{
       0,    49,    49,    58,    65,    74,    81,    93,    92,   116,
     115,   140,   151,   162,   174,   190,   189,   221,   220,   248,
     259,   266,   273,   282,   294,   307,   319,   335,   342,   351,
     358,   366,   373,   380,   387,   394,   401,   408,   415,   424,
     431,   440,   448,   457,   464,   473,   480,   489,   496,   505,
     513,   522,   530,   540,   547,   554,   563,   570,   577,   584,
     591,   598,   605,   614,   622,   630,   637
};
#endif
...
#line 93 "22101348.y"
			...
			symbol_info* f = new symbol_info(yyvsp[-3]->get_name(), "ID");
			f->set_ID_type("function");
			f->set_var_type(yyvsp[-4]->get_name());
...
#line 116 "22101348.y"
                {
			inject_params_next_scope = true;
			symbol_info* f = new symbol_info(yyvsp[-2]->get_name(), "ID");
			f->set_ID_type("function");
			f->set_var_type(yyvsp[-3]->get_name());
			f->set_param_type(param_types);
			f->set_param_name(param_names);
			if (!symtbl->insert(f)) delete f;
		}
...
#line 141 "22101348.y"
			...
			param_types.push_back(yyvsp[-1]->get_name());
        	param_names.push_back(yyvsp[0]->get_name());		
...
#line 152 "22101348.y"
			...
			param_types.push_back(yyvsp[0]->get_name());
...
#line 283 "22101348.y"
			...
			symbol_info* s = new symbol_info(yyvsp[0]->get_name(), "ID");
...
#line 295 "22101348.y"
            ...
            // you may need to store the variable names to insert them in symbol table here or later
			symbol_info* s = new symbol_info(yyvsp[-3]->get_name(), "ID");
        	s->set_ID_type("array");
        	s->set_var_type(current_decl_type);
        	s->set_arr_size(std::stoi(yyvsp[-1]->get_name()));
